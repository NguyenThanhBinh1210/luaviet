...
export const AccodionFilter = ({ title, children, className, headerClassName }: { headerClassName?: string, title: string; children: React.ReactNode, className?: string }) => {
  const [show, setShow] = useState(false)
  return (
    <div className={` ${className} pb-3 pt-4 border-b mb-3`}>
      <div
        onClick={() => setShow(!show)}
        className=' flex  items-center justify-between text-[#013879] cursor-pointer mb-2'
      >
        <p className={`${headerClassName} font-bold`}>{title}</p>
        <svg
          xmlns='http://www.w3.org/2000/svg'
          fill='none'
          viewBox='0 0 24 24'
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 transition-all overflow-hidden ${show ? 'max-h-[4000px]' : 'max-h-0'}`}>
        {children}
      </div>
    </div>
  )
